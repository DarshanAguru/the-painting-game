...
def opps(request):
    ...
    if request.session.has_key('clueid'):
        if request.session['clueid'] == 10 or request.session['clueid'] == 11:
            return render(request,'opps.html',{"clue":"Oops!!! you got Trapped.. It was a trap setup by the king. It seems the question had other answers too which we were unable to figure out. Oh Nooo! the door closed, we are now trapped in this room forever :( . Click on restart to play again."})
...
def gamest(request):
    ...
    if(not request.session.has_key('clueid')):
         ...
         request.session['sttime'] = str(time.time())
         request.session['echtime'] = str(time.time())
    if request.session['clueid'] > 10:
        return redirect('/opps/')
    psk = request.session['clueid']-1 #10
    lst= Clues.objects.all()
    print("#0", request.session['clueid'])
    print("#-1", psk)
    
    # psk = psk if psk < len(lst) else len(lst)-1
    ...
    if request.method=="POST":
         ...
         if psk == 4:
            ...
            print("#1",request.session['clueid'])
            request.session['clueid'] = int(request.session['clueid'])+1 #11
            print("#2",request.session['clueid'])
            psk=int(request.session['clueid'])-1 #10
            print("ERRRRR:",psk)
            request.session['echtime'] = str(time.time())
